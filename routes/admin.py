...
def users():
    ...
    # for user in users:
    #     recent_log = Logs.query.filter_by(user_id=user.user_id).order_by(Logs.timestamp.desc()).first()
...
# UPDATE USER DETAILS
@admin_bp.route('/update-user/<string:user_id>', methods=['POST'])
@login_required
def update_user(user_id):
    ...
    # user.office_id = data.get('department', user.office_id)
...
# DELETE USER
@admin_bp.route('/delete-user/<string:user_id>', methods=['POST'])
@login_required
